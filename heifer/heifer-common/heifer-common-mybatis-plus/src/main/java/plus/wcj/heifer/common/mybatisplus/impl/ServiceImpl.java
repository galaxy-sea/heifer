...
public class ServiceImpl<M extends BaseMapper<T>, T, ID extends Serializable> implements IService<T, ID> {
    ...
    @Override
    public boolean update(T updateWrapper) {
        return update(updateWrapper, null);
    }

    @Override
    public boolean update(T entity, T updateWrapper) {
        return SqlHelper.retBool(getBaseMapper().update(entity, new QueryWrapper<>(updateWrapper)));
    }
