...
CREATE TABLE `rbac_user` (
    ...
    `is_account_non_expired` tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '指示用户的帐户是否已过期。 过期的帐户无法通过身份验证。\n返回值：\ntrue如果用户的帐户是否有效（即未过期）， false如果不再有效（即到期）',
    `is_account_non_locked` tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '指示用户是锁定还是解锁。 锁定的用户无法通过身份验证。\n返回值：\ntrue如果用户没有被锁定， false否则',
    `is_credentials_non_expired` tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '指示用户的凭据（密码）是否已过期。 过期的凭据会阻止身份验证。\n返回值：\ntrue如果用户的证书是有效的（即非到期）， false如果不再有效（即到期',
    `is_enabled` tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '指示用户是启用还是禁用。 禁用的用户无法通过身份验证。\n返回值：\ntrue如果用户已启用， false否则',
    `create_by` bigint(20) UNSIGNED NOT NULL,
    `update_by` bigint(20) UNSIGNED NULL,
    `create_time` timestamp DEFAULT CURRENT_TIMESTAMP,
    `update_time` timestamp ON UPDATE CURRENT_TIMESTAMP,
